...
const Confirm = () => {
  ...
  const {user} = useAuth(); // Directly using useContext without parsing
  ...
  return (
    <div>
      ...
      {error && (
        <Alert
          message={
            "Oops! Looks like there was an error while booking your appointment, please try again."
          }
        />
      )}
      ...
      <div className="flex justify-center">
        <div className="bg-white border-2 border-black w-[50%] p-5 rounded-base shadow-dark">
          <h3 className="text-text font-heading font-body text-lg mb-4">
            ...
            {`${Centre.Name}, ${Centre.Address}, ${Centre.Postcode}`}
          </h3>
          <h3 className="text-text font-heading font-body text-lg mb-4">
            <span>Date:</span>{" "}
            {`${days[appointmentDate.getUTCDay()]}, ${appointmentDate.getDate()} ${months[appointmentDate.getUTCMonth()]} ${appointmentDate.getUTCFullYear()}`}
          </h3>
          <h3 className="text-text font-heading font-body text-xl">
            <span>Time:</span>{" "}
            {`${appointmentTime.getUTCHours()}:${appointmentTime.getUTCMinutes().toString().padStart(2, "0")}`}{" "}
            (24-hour time)
          </h3>
          <div className="flex justify-center">
            <Button
